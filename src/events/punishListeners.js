...
module.exports = (client) => class BotListeners extends ListenerAdapter {
  ...
  async onReactionAddListener(reaction, user) {
    ...
    if (reaction.emoji.name == '✅') {
      ...
      reaction.message.edit(new MessageEmbed().setColor(embed.color).setThumbnail(embed.thumbnail.url).setFooter({ text: embed.footer.text }).setDescription(`${embed.description}\n\nPunição aplicada em: **${formatDateBR(Date.now())}**\npor ***${user.username}***.`).setAuthor(embed.author.name, embed.author.proxyIconURL));
      ...
      acceptChannel.send(new MessageEmbed()
        ...
        Aceita por **${user.username}**.`).setFooter({ text: 'Denúncia respondida em ' + formatDateBR(Date.now()) }))
      ...
      denyChannel.send(new MessageEmbed()
        ...
        Negada por **${user.username}**.`).setFooter({ text: 'Denúncia respondida em ' + formatDateBR(Date.now()) }))
      reaction.message.delete();
    }
  }
}
