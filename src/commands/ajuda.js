...
exports.run = async (client, message, args, command) => {
    ...
    await painel.edit('Coletamos as permissões, carregando painel...', new MessageEmbed()
        ...
        .setColor(`#36393f`))
    await painel.delete()
    if (message.member.roles.highest.rawPosition <= (helperRole && helperRole.rawPosition)
        ...
        await message.channel.send({
            embeds: [new MessageEmbed()
                ...
                .setColor(`#36393f`)]
        }).then(async message => { try { setTimeout(() => message.delete(), 10000) } catch (error) { } });
    } else {
        await message.channel.send({
            embeds: [new MessageEmbed()
                .setTitle(`Confira os comandos disponíveis para você!`)
                ...
                .setColor(`#36393f`)]
        }).then(async message => { try { setTimeout(() => message.delete(), 1000) } catch (err) { } });
    }

    try { setTimeout(painel.delete(), 1000 * 10) } catch (error) { }
